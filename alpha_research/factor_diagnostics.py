...
def generate_factor_diagnostics(
    *,
    ...
    return {"timeseries": timeseries, "statistics": statistics, "decay": decay, "similarity": similarity, "clusters": clusters, "families": families, "stability": stability}
...
def _statistics(timeseries: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for factor, day in daily.groupby("factor", sort=True):
        ...
        rows.append({"factor":factor,"mean_IC":mean,"IC_std":std,"ICIR":mean/std if std>1e-12 else 0.0,"IC_t_stat":mean/(std/np.sqrt(len(ic))) if std>1e-12 and len(ic)>1 else 0.0,"positive_IC_ratio":float((ic>0).mean()) if len(ic) else 0.0,"observations":int(day["observations"].sum())})
